...
jest.mock('../../../toolbar/native', () => {
  const actual = jest.requireActual('../../../toolbar/native');
  return {
    ...actual,
    RouterToolbarItem: jest.fn((props) => null),
  };
});

jest.mock('../../../link/preview/native', () => {
  const actual = jest.requireActual('../../../link/preview/native');
  return {
    ...actual,
    NativeLinkPreviewAction: jest.fn((props) => null),
  };
});
  ...
  describe('Allowed children validation', () => {
    describe('Button with Label, Icon, Badge children', () => {
      ...
      it('correctly passes Label and Icon to native component in bottom placement', () => {
        ...
        expect(screen.getByTestId('index')).toBeVisible();
        // Verify RouterToolbarItem was called with correct props
